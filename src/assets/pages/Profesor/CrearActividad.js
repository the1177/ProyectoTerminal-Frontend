...
const ActividadContent = () => {
  ...
  async function enviarPeticion(values) {
    ...
    datos.append("cursoId", objCurso.cursoId ? objCurso.cursoId : 6);
    datos.append("nombre", values.tituloActividad ? values.tituloActividad : 'Sin titulo');
    datos.append("descripcion", values.descripcionActividad ? values.descripcionActividad : 'Sin descripcion');
    ...
    //datos.append("criterios", values.puntos ? values.puntos : 100);
    datos.append("puntosMax", values.puntos ? values.puntos : 100);
    //datos.append("aaa", aaa);
    ...
    try {
      await axios.post(BackendURL + "/api/misiones", dataJson, {
        headers: {
          'Content-Type': 'application/json'
        }
      }).then(response => {
        // Actividad Guardado OK
        if (response.data === "Actividad agregada exitosamente.") {
          console.log(response);
          alert("Actividad agregada exitosamente");
          redirigirCrearActividad();
        }
